  ...
  try {
    ...
    if (role === 'barber') {
      try {
        ...
        // Handle all trial/free plan variants (trial-2-months, trial-3-months, free_trial, etc.)
        if (!subscriptionPlan || subscriptionPlan.includes('trial') || subscriptionPlan.includes('free')) {
          const expiry = new Date(now);
          // Extract month count from plan name (e.g. 'trial-2-months' → 2, 'trial-3-months' → 3)
          const monthMatch = subscriptionPlan?.match(/(\d+)/);
          const trialMonths = monthMatch ? parseInt(monthMatch[1], 10) : 2; // Default 2 months
          expiry.setMonth(now.getMonth() + trialMonths);
          trialUsed = false;
          subscriptionStatus = 'free_trial';
          subscriptionExpiryDate = expiry.toISOString();
          console.log(`[SIGNUP] Trial plan detected: ${subscriptionPlan || 'default'}, duration: ${trialMonths} months, expiry: ${subscriptionExpiryDate}`);
        }
        ...
        // Normalize plan_type to values accepted by subscriptions_plan_check constraint
        // Trial variants (trial-2-months, trial-3-months, free_trial) all map to 'free_trial'
        const normalizedPlanType = (!subscriptionPlan || subscriptionPlan.includes('trial') || subscriptionPlan.includes('free'))
          ? 'free_trial'
          : subscriptionPlan;
...
// Yandex Maps API key endpoint
app.get("/make-server-166b98fa/ym-api-key", (c) => {
  const key = Deno.env.get('YM_API_KEY') ?? '';
  if (!key) {
    console.log('[YM] ❌ YM_API_KEY not configured');
    return c.json({ error: "YM_API_KEY not configured" }, 500);
  }
  return c.json({ key });
});
