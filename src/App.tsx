...
function AppContent() {
    ...
    try {
      ...
      if (role === 'barber') {
        ...
        const userFromDatabase = {
          ...
          workplaceCoordinates: (() => {
            try {
              if (barberData.google_maps_url) {
                const parsed = JSON.parse(barberData.google_maps_url);
                if (parsed?.lat && parsed?.lng) return { lat: parsed.lat, lng: parsed.lng };
              }
            } catch { /* Not JSON coords, ignore */ }
            return undefined;
  ...
  const handleToggleFavorite = async (barberId: string) => {
    ...
    // Prevent spam clicking - check if this barber is already being processed
    if (pendingFavorites.has(barberId)) {
      if (process.env.NODE_ENV === 'development') {
        console.log('[FAVORITES] Already processing this barber, ignoring click');
      }
      return;
    }
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log('[FAVORITES] Optimistic toggle favorite:', { 
        barberId, 
        exists, 
    ...
    const newFavoriteIds = exists 
      ? favoriteIds.filter(id => id !== barberId)  // Remove
      : [...favoriteIds, barberId];                 // Add
    
    setFavoriteIds(newFavoriteIds);
    
    // Mark as pending to prevent spam clicks
    setPendingFavorites(prev => new Set(prev).add(barberId));
    ...
    try {
       ...
       if (exists) {
         // DELETE from favorites via backend
         if (process.env.NODE_ENV === 'development') {
         ...
         if (!response.ok) {
           ...
           setFavoriteIds(favoriteIds);
           toast.error(t('toast.failedToUpdateFavorites') || "Failed to update favorites. Please try again.");
           return;
         }
         ...
         // No success toast for better UX (action was already visual)
       } else {
         // INSERT into favorites via backend
         if (process.env.NODE_ENV === 'development') {
       ...
       // SUCCESS: Keep the optimistic state (no need to re-fetch)
       if (process.env.NODE_ENV === 'development') {
         console.log('[FAVORITES] ✅ Optimistic update confirmed by backend');
       }
    } catch (e) {
