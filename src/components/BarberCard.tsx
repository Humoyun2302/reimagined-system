...
export function BarberCard({ barber, onBookNow, showShareButton = true, isFavorite = false, isPendingFavorite = false, onToggleFavorite, onShowOnMap }: BarberCardProps) {
    ...
    >
      <Card className="overflow-hidden group relative rounded-3xl border border-gray-100 dark:border-gray-700/50 hover:border-[#5B8CFF]/30 transition-all duration-200 bg-white dark:bg-gray-800 shadow-sm hover:shadow-md">
              ...
              <div className="absolute top-3 right-3 z-20 flex items-center gap-2">
                ...
                {onToggleFavorite && (
                  <Button
                    ...
                    className="gap-1 bg-white/90 hover:bg-white backdrop-blur-sm bg-[rgba(255,255,255,0.6)] text-[rgb(0,0,0)] disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <Heart 
                      className={`w-3 h-3 transition-all duration-300 ease-in-out ${
                        isFavorite ? 'fill-red-500 text-red-500 scale-110' : 'text-gray-600 scale-100'
                      } ${isPendingFavorite ? 'opacity-50' : 'opacity-100'}`}
                    />
                  </Button>
                )}
        ...
        <CardContent className={`p-4 space-y-4 rounded-[20px] ${!hasValidImages ? 'pt-6' : ''}`}>
              ...
              <div className="flex items-center gap-2">
                ...
                {onToggleFavorite && (
                  <Button
                    ...
                    disabled={isPendingFavorite}
                    className="gap-1 bg-gray-100 hover:bg-gray-200 text-[rgb(0,0,0)] disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <Heart 
                      className={`w-3 h-3 transition-all duration-300 ease-in-out ${
                        isFavorite ? 'fill-red-500 text-red-500 scale-110' : 'text-gray-600 scale-100'
          ...
          {/* Toggle Button & Book Now - Show toggle if there's expandable content */}
          <div className="flex items-center justify-between gap-3 pt-3 border-t border-gray-100 dark:border-gray-700">
            {(barber.description || barber.workplaceCoordinates || barber.barbershopName || barber.servicesForKids) && (
              <Button
                variant="ghost"
                size="sm"
            ...
            <Button
              ...
              className={`gap-2 bg-gradient-to-r from-emerald-600 to-blue-600 hover:from-emerald-700 hover:to-blue-700 transition-all duration-300 text-xs sm:text-sm px-3 sm:px-4 ${
                (barber.description || barber.workplaceCoordinates || barber.barbershopName || barber.servicesForKids) ? 'flex-[0.85]' : 'flex-1'
