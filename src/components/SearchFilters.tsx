...
export function SearchFilters({ barbers, isBarbersLoading = false, onBookNow, favoriteIds, pendingFavorites, onToggleFavorite, priorityBarberId, onShowOnMap }: SearchFiltersProps) {
  ...
  const hasScrolledToPriority = useRef(false);
  
  useEffect(() => {
    // Reset the flag when the priority barber changes (new navigation)
    hasScrolledToPriority.current = false;
  }, [priorityBarberId]);

  useEffect(() => {
    if (priorityBarberId && priorityBarberRef.current && !isBarbersLoading && !hasScrolledToPriority.current) {
      hasScrolledToPriority.current = true;
